...
7. **Phase 5**: 썸네일 이미지 프롬프트 **계열별 9개씩** 생성. 어느 계열을 만들지는 **`config/thumbnail-strategy.json`의 `styles` 배열**이 정한다(실사형·국기볼형·정세형·일러스트형 중). 계열마다 파일 분리
...
5. **auto 모드**: 최적안 확정 → concept.md + hook-intro.md 저장 → Phase 5 실행
...
- **auto 모드**: `{P}/_script/concept.md` + `{P}/_script/hook-intro.md`
  + **`styles`에 적힌 계열마다 JSON 1개** — `prompts.json`(실사형 9) / `prompts-countryball.json`(국기볼형 9) /
    `prompts-geopolitics.json`(정세형 9 — 국가·안보 소재일 때만) / `prompts-illustration.json`(일러스트형 9)
  + ⛔ `styles`에 없는 계열은 만들지 않는다. "항상 만드는 계열"은 없다
...
- **썸네일 계열 분리**: 실사형(실존 인물 실명 필수·**자연광 다큐 사진 톤**) / 국기볼형(실존 인물 절대 금지·애니메 구체) / 정세형(국가원수 실명·**실사 사진 + 시사 그래픽 합성**, 국가·안보 소재 전용) / **일러스트형**(마스코트·실존 인물 금지·채널 전용 규칙 파일). 각 규칙 파일을 따로 읽고 따로 적용하며 한쪽 규칙을 다른 쪽에 섞지 않는다. **정세형은 서사 방향(부정/긍정)을 먼저 판정**하고, 소재가 해당하지 않으면 생성하지 않는다.

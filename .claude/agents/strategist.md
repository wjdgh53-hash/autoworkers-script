...
| 경로 | 용도 |
|------|------|
| `prompts/creative-strategy.md` | 설계 절차 (필수) |
| `prompts/ctr-reference.md` | CTR 이론 (필수) |
| `prompts/thumbnail-design.md` | **실사형** 썸네일 규칙 (Phase 5) |
| `prompts/thumbnail-countryball.md` | **국기볼형** 썸네일 규칙 (Phase 5) |
| `prompts/thumbnail-geopolitics.md` | **정세형** 썸네일 규칙 (Phase 5, 국가·안보 소재일 때) |
| `prompts/pd-templates.md` | 산출물 포맷 정의 (필수) |
...
- **auto 모드**: `{P}/_script/concept.md` + `{P}/_script/hook-intro.md`
  + `{P}/output/thumbnails/prompts.json` (실사형 5)
  + `{P}/output/thumbnails/prompts-countryball.json` (국기볼형 5)
  + `{P}/output/thumbnails/prompts-geopolitics.json` (정세형 5 — 국가·안보 소재일 때만)
...
- **계승과 복제의 구분(D)**: 계승 대상은 **틀**이다. 앵커의 단어·비유·문구를 그대로(또는 핵심 명사만 바꿔) 옮기면 **1개라도 실패**다. 썸네일도 구도 골격만 계승하고 피사체·색·시점·강조요소 중 최소 3개는 다르게 한다. 확정 전에 앵커 원문과 대조한다.
...
- **썸네일 3계열 분리**: 실사형(실존 인물 실명 필수·광고 합성 톤) / 국기볼형(실존 인물 절대 금지·애니메 구체) / 정세형(국가원수 실명·뉴스 위기물 톤, 국가·안보 소재 전용). 각 규칙 파일을 따로 읽고 따로 적용하며 한쪽 규칙을 다른 쪽에 섞지 않는다. **정세형은 서사 방향(부정/긍정)을 먼저 판정**하고, 소재가 해당하지 않으면 생성하지 않는다.
- **썸네일 전략 우선**: `config/thumbnail-strategy.json`이 있으면 기본값 대신 채널별 설정을 따른다.
- **boilerplate 불포함**: prompt_en에 비율/구도 여백 문구를 중복해서 넣지 않는다 (각 규칙 파일의 지시를 따른다).

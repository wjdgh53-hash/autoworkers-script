...
4. 🚨 **auto 모드였으면 「패키지 확정 근거」를 함께 낸다** (2026-08-29 신설 — 아래 절 참조)
5. 글자수, 예상 분량(~440자/분) 표시
6. **완성본 폴더를 연다 (2026-08-25 신설).** 보고 마지막에 OS에 맞게 `{P}/output`을 파일 탐색기로 띄운다.
...
### 🚨 패키지 확정 근거 — auto 모드였을 때만 (2026-08-29 신설)

**언제 내는가**: `workflow.json`의 `mode`가 `auto`여서 **PD가 패키지를 묻지 않고 확정한 경우.**
⛔ ask 모드였으면 내지 않는다. 사용자가 직접 고른 것을 되돌려 주는 셈이라 군더더기다.

**왜 내는가**: auto 모드는 STRATEGY에서 멈추지 않으므로 **사용자가 확정 지점을 볼 창이 없다.**
그런데 이 지점에서 갈리는 것이 크다 — 앵커가 주 레퍼가 아닌 쪽으로 잡히거나, 주 레퍼의 전제가 팩트체크에서 무너져 서사를 계승하지 못하는 일이 실제로 있었다(2026-08-29 방구석 N8).
**사용자는 파이프라인이 도는 동안 화면을 보고 있지 않다.** 중간에 내면 스크롤에 묻히므로 **완료 보고에 낸다.**

**형식** — 산출물 5줄과 레퍼 세트 표 **다음에** 낸다. 4줄을 넘기지 않는다.
...
**고른 이유**: {2문장 이내}
**⚠️ 이 편에서 갈린 판단**: {설계를 바꾼 사건이 있으면 1~2줄. 없으면 이 줄을 뺀다}
```

- **점수는 `_strategy_candidates.md`의 자체 평가 종합 점수**를 그대로 옮긴다. 새로 매기지 않는다
- **4개 패키지를 전부 늘어놓지 않는다.** 확정안과 차순위 하나면 충분하다 — 나머지는 `_strategy_candidates.md`에 남아 있다
- 🔴 **「이 편에서 갈린 판단」에 반드시 적을 것**: 앵커가 주 레퍼가 **아닌** 레퍼로 잡혔다 / 주 레퍼의 핵심 전제가 팩트체크에서 무너졌다 / 대기열의 제목 원안을 폐기했다 / 소재를 기각하고 다음 순번으로 내려갔다 — **이 넷 중 하나라도 있으면 빠뜨리지 않는다.** 사용자가 나중에 성적을 해석할 때 필요한 정보다

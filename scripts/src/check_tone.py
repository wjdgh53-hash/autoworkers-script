...
기준값 (2026-08-19 현재. 수치의 유일한 원본은 아래 _CEILINGS·_FLOORS·_CHANNEL_BANDS다):
    ...
    | 1인칭          | ≤ 1.2 (FAIL)  | 동일           |
    | 한 문장 나열   | 4개부터 FAIL   | 동일           |
    | 비유 소재      | ≤ 3.0 (주의)   | 동일           |
    | 반문·권위 인용·전환어·2인칭 | 측정만 (기준 없음) | 동일 |

    ⚠️ **채널별로 다른 값은 「숫자 밀도 하한」 하나뿐이다.** 나머지는 전부 공통이다.
...
#
# 🔴 2026-08-19 — 이중 계수 버그를 고쳤다.
#    "5만 7천 명"은 _NUM_RE가 "5만 7천"으로 한 번 세는데, _KO_NUM_RE가 뒤쪽 "천 명"을
#    또 세서 **한 값이 두 개로 잡혔다.** 우리 대본은 큰 수를 이렇게 아라비아+한글로
#    섞어 쓰므로("6만 6천 명", "1조 8천억 원") 오탐이 광범위했다.
#    → 앞에 아라비아 숫자가 붙어 있으면(사이에 공백·쉼표만 있어도) 한글 수사로 세지 않는다.
...
_DEVICES = {
    # 반문. ⚠️ 2026-08-19부터 **기준이 없다**(위 _FLOORS 주석 참조). 값만 재서 보고서에 싣는다.
    ...
    #
    # ⚠️ 2026-08-19 — 1인칭이 아닌 말 4개(솔직히·화가 나·놀랐·믿기지)를 뺐다.
    #    이 넷은 화자를 가리키지 않고 그냥 부사거나 3인칭 서술이라 오탐이 났다:
    #      "솔직히 흔한 일이 아니에요"        ← 부사
    #      "화가 나서 문을 두드려도"           ← 3인칭 장면 묘사
    #      "믿기지 않는 수치입니다"           ← 3인칭
    #    실측: 이 넷 때문에 16편이 걸렸는데 진짜 1인칭 기준으로는 12편이었다.
    #    '얼떨떨'은 사실상 화자 상태 서술이라 남긴다.
    "1인칭": re.compile(r"(저도 그랬|저는 좀|저도 처음|얼떨떨|제가 |저는 |저도 )"),
...
#      → 반문은 **어느 채널에도 기준을 두지 않는다.** 값은 재서 보고서에만 싣는다.
#
...
_CHANNEL_BANDS: dict[str, dict[str, dict[str, float]]] = {
    "bangguseok-economy": {
        # 이 채널의 상한은 **하나도 없다.**
        #
        # 🔴 2026-08-19 — 여기 있던 "반문 4.0" · "비유 소재 4.0 FAIL"을 **같은 날 철회했다.**
        #    둘 다 08-16 사고를 막으려고 넣은 것인데, 그 사고의 원인은 규칙 쪽이었고
        #    원인은 이미 전부 제거됐다:
        #      · 반문 7.7  ← 기본 하한 3.5 + "60초에 한 번꼴로 되묻어라" 지시 (삭제됨)
        #      · 비유 4.6  ← 확장 비유가 8블록 필수 + "판정이 아니라 볼 자리 표시" 문구 (둘 다 삭제됨)
        #    원인을 지운 뒤에 결과를 막는 상한을 또 두는 것은, 이 저장소에 규칙이 쌓여 온
        #    바로 그 방식이다. 대본이 이상해진 원인이기도 하다.
        #    → 넣지 않는다. 같은 결함이 **실제로 재발하면** 그때 근거를 갖고 다시 판단한다.
        "ceilings": {},
        # 12.0 = 결이 같은 벤치 7편 중 최저(사연형 12.2) 바로 아래.
        # 히트 보장선이 아니라 "비유로 분량을 때웠나"의 진단 지표다.
        # 미달은 WARN이며 차단하지 않는다.
        "floors": {"숫자 밀도": 12.0},
    },
}
...
_UNIT = (
    r"(?:원|달러|유로|엔|명|개|대|문|발|기|척|건|곳|톤|퍼센트|%|배|"
    r"km|킬로미터|미터|시간|분|초|년|개월|나노|캐럿|접시)?"
)
_RANGE_RE = re.compile(
    r"\d[\d,.]*\s*(?:[조억만천]\s*\d[\d,.]*)*\s*(?:[조억만천])?\s*" + _UNIT + r"\s*"
    r"(?:에서|~|-|부터)\s*"
    r"\d[\d,.]*\s*(?:[조억만천]\s*\d[\d,.]*)*\s*(?:[조억만천])?\s*" + _UNIT
)

# 비율은 두 수처럼 보이지만 하나의 값이다 — "6대 1", "2대 1 안팎", "59대 16".
# 분수와 같은 논리다(위 _FRACTION_RE 참조).
_RATIO_RE = re.compile(r"\d[\d,.]*\s*대\s*\d[\d,.]*")

...
## §2. 스타일 — 경제해적단을 백지에서 그대로 복제한다

> 🔴 **2026-08-20 2차 전면 개정.** 1차 개정은 기존 씬 설명에 색·표정만 얹어서 **전혀 닮지 않았다.**
> 정호님: *"기존의 스타일 완전히 백지화 시키고 완전히 똑같이 만들어라. 기존에 있던 건 완전히 버려."*
> 이 절은 **참고가 아니라 복제 사양**이다. 아래를 그대로 따른다.

### 실측 — 경제해적단 상위 6편 (160×90 축소 후 HSV)
...
| **소품 크기** | **글자가 붙은 것은 현실 비율을 무시하고 크게 그린다.** 손에 든 폰이 머리만 하고, 배지가 얼굴 3분의 1만 하다 |

> 🔴 **원본은 [prompts/thumbnail-design.md](../../../prompts/thumbnail-design.md)의 「글자가 붙은 소품은 크게」 절이다 (3계열 공통).**
> 여기에 다시 적지 않는다 — 일러스트형은 **현실 비율을 무시하고 그리는 쪽**이다.
| **마스코트** | §3 |
...
- ⛔ **빈 화면을 만들지 않는다.** 벤치는 어느 구석에도 볼 것이 있다 — 카드가 3개 미만이면 다시 만든다

## §3. 마스코트 — 경제해적단 캐릭터를 그대로 복제한다

> 🔴 **2026-08-20 2차 개정.** 앞서 「눈썹까지 따라하진 말라」는 지시로 눈썹을 뺐더니
> 결과물이 **점 눈 무표정**으로 나와 벤치와 전혀 닮지 않았다.
> 정호님 최종 지시: **"완전히 똑같이 새로 만들어."** → 얼굴도 복제한다.

### 고정 사양 (9장 전부 동일)

```
a large smooth round off-white head with no hair and no nose, drawn with a bold black outline;
two big white eyes with round black pupils and visible eyelids; thick solid black eyebrows that
change angle with the emotion; a drawn mouth that opens and bends with the emotion; a small
simple body in situation-appropriate clothing with simple rounded hands
```

| 항목 | 사양 |
|---|---|
| **머리** | **크다.** 화면 높이의 **30~40%.** 매끈한 미색 원형, 머리카락·코 없음, 굵은 검정 외곽선 |
| **눈** | **큰 흰자 + 검은 원형 눈동자.** ⛔ 작은 점 눈이 아니다 — 1차 실패의 원인 |
| **눈썹** | **굵은 검정.** 감정의 1차 신호 — V자(분노·경고) / 八자(불안·낭패) / 아치(놀람) / 평평(담담) |
| **입** | 감정에 따라 연다 — 미소 곡선 / 벌린 O / 아래로 꺾인 찡그림 / 짧은 선(담담) |
| **몸** | 상황복. **정장이 기본**이고 작업복·등산복 등으로 바뀐다. 손은 단순한 둥근 모양 |
| **소품** | 선글라스 · 모자 · 커피잔 · 서류 — 여유/낭패를 소품으로도 말한다 |
| **표정 부속** | **땀방울 · 눈물 · 머리 감싼 손 · 이마 짚기 · 가리키는 손** |

> **실측**: [부자들은 주식을 팔지 않습니다](https://www.youtube.com/watch?v=aepM7E3vsTs)는 한 얼굴에 **땀방울 + 머리 감싼 손 + 벌린 입**을 다 쓰고, 옆 캐릭터는 **선글라스 + 커피잔**으로 여유를 표현한다.
> [배당주 투자](https://www.youtube.com/watch?v=vrZKuTGJsQU)는 **선글라스 + 밀짚모자 + 커피잔 + 흔들의자**로 「이미 이룬 사람」을 한 장에 세운다.
> [달러 패권 157,000회](https://www.youtube.com/watch?v=NID7VESSER8)는 **크게 뜬 눈 + 벌린 입 + 손바닥 편 제지 동작**이다.

> 🚨 **한 프로젝트 안 9장의 얼굴은 반드시 동일하다.** 몸과 표정만 바뀐다.
...
### 🔴🔴 큰 글자로 넣을 금액은 **6글자 이내**로 줄인다 (2026-08-22 실측)

> **실패**: `"100,000,000원"`(12글자)을 「각 글자가 프레임 높이의 1/10」로 요구했더니
> 모델이 **`100,00` / `0,000원`으로 자릿수 중간을 갈라** 두 줄에 앉혔다. 금액으로 읽히지도 않는다.
> 정호님이 직접 **`1억 원`(3글자)** 으로 고쳐 해결했다.
> **원인은 「얼마나 크게」만 있고 「몇 글자까지」가 없었던 것**이다 — 글자 하나가 화면 높이의 1/12 이상이면 12글자는 물리적으로 한 줄에 안 들어간다.

| | 큰 글자 자리 (화면 높이 1/12 이상) | 잔글씨 자리 |
|---|---|---|
| ⭕ | **한글 단위로 줄인다** — `"1억 원"` · `"9,960만 원"` · `"230만 원"` | 원단위 그대로 — `"81,000원"` · `"211,500원"` |
| ⛔ | 아라비아 9자리 — `"100,000,000원"` | — |

- **게이트 B(원단위 금액)는 이걸로 깨지지 않는다.** 큰 글자는 한글 단위로 두고, **같은 화면 안 잔글씨 행에 원단위를 병기**하면 둘 다 만족한다(정호님 수정본이 정확히 그 구조다 — 큰 `1억 원` + 작은 종목별 `81,000원`)
- 프롬프트에 **`always on ONE unbroken line, never split in the middle of a digit group`을 반드시 붙인다.** 공통 블록 `NUMBER RULE`이 이 문장을 담당한다

### 🔴 폰·모니터 화면은 **밝은 흰 UI**다. 비워 두지 않는다 (2026-08-22 실측)

> **실패**: 앵커 계승 규칙(「그래프·화살표 없이 얼굴만」)을 **소품 내부 UI에까지 적용**해
> `no chart and no arrow on the screen`이라고 썼다. 그 결과 **검은 화면에 글자만 뜬 휑한 폰**이 나왔고,
> 어두운 하단 세이프존과 붙어 한 덩어리가 됐다.
> 🔴 그 규칙은 **화면 전체 구도**에 대한 것이지 소품 안쪽에 대한 것이 아니다. 과잉 적용이었다.

- 폰·노트북 **계좌 화면은 화면에서 가장 밝은 사각형**이다. `BRIGHT WHITE account app interface, the brightest rectangle in the picture`
- 화면 안을 **실제 앱처럼 채운다**: 작은 헤더(`"주식 계좌 잔고"`) → **큰 잔액**(`"1억 원"`) → 작은 증감률(`"+3.2%"`) → 작은 상승 차트선 → 하단 **빈 리스트 행 + 네비 아이콘**(글자 없음)
- ⛔ **`cursor bar mid-blink` 같은 장식을 넣지 않는다** — 게이트 C(사건 진행중)를 만족시키려 넣었으나 실제로는 정체불명의 흰 막대가 하나 생겼을 뿐이다. 진행중은 **「잔액이 방금 갱신됐고 차트선이 아직 그려지는 중」**으로 만든다
- ⚠️ **TV 뉴스 자막은 예외** — 현실에서도 어두운 바탕에 흰 글자다. 그대로 둔다

### 🔴 글자를 안 넣을 소품에는 그 자리에서 **`blank`를 붙인다** (2026-08-22 실측)

> **실패**: `A round golden badge`라고만 쓰고 문구를 지정하지 않았더니
> 모델이 배지에 **「강정」이라는 깨진 한글**을 채워 넣었다.
> 프롬프트 **끝의 일괄 금지문**("다른 표면엔 글자 없음")은 **개별 소품을 못 막는다.**

- **글자를 넣을 소품**: 전부 큰따옴표로 문구를 지정한다. 배지 `"수익 달성"` · 천장 간판 `"나의 투자 포트폴리오"` · 게시물 `"시장 동향"`·`"투자 유망주"`
- **글자를 안 넣을 소품**: 그 사물을 쓴 문장 안에 **`blank`를 직접 박는다** — `every other sheet on that board is blank paper`
- 공통 블록 `LETTERING RULE`이 「지정한 것 외 전부 무지」를 한 번 더 못 박는다

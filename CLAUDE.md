...
| **URL 없음** + "대본 만들어줘 / 이어서 해줘 / 다시 써줘" | `script-pd` (로컬 프로젝트 재개) | |
| **URL 없음** + "{주제}로 대본 만들어줘" | `script-pd` — 주제가 있으므로 레퍼를 찾아 COLLECT | 무엇을 만들지가 정해져 있다 |
| **URL 없음** + "다음 컨텐츠 기획 / 채널 진단" | `channel-trend-pd` (채널 URL 물어봄) | |
| **대기열 소진** — "대기열 다음 거 만들어줘 / 큐에서 다음 소재로 / 다음 순번 제작" | `channel-trend-pd` **빠른 경로** — `production-queue.md`의 미제작 1순위를 꺼내 배율만 재확인하고 곧장 `script-pd` 인계 | **이미 검증된 소재다. 전체 스캔을 다시 돌리지 않는다** |
